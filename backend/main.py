...
from ulpin_generator import parse_3d_ulpin, compute_checksum
...
def get_unit(ulpin: str):
    ...
    for idx, unit in enumerate(units):
        if unit['ulpin'] == ulpin:
            ...
            # Derivation provenance, computed from existing data (no invented values)
            try:
                parsed = parse_3d_ulpin(ulpin)
                checksum_valid = compute_checksum(f"{parsed['parent_ulpin']}-{parsed['layer']}-{parsed['level']}-{parsed['unit']}") == parsed['checksum']
            except ValueError:
                parsed = None
                checksum_valid = False
            
            provenance = {
                "derivation": "Generated by prototype geometry engine from building_config.json + parcel.geojson (Controlled Demo Data)",
                "ulpin_components": parsed,
                "checksum_valid": checksum_valid,
                "ownership_note": "Ownership/transaction history not included in demo dataset",
                "parcel_id": load_json('parcel.geojson')['features'][0]['properties']['parcel_id']
            }
