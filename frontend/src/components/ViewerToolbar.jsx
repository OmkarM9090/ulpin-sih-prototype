    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
        <div style={{ position: 'relative' }}>
          <button 
            style={{ display: 'flex', gap: '8px', alignItems: 'center', backgroundColor: 'var(--bg-elevated)', color: 'var(--text-primary)', border: '1px solid var(--border-default)' }}
            onClick={() => setPresetOpen(!presetOpen)}
          >
            {PRESETS.find(p => p.id === cameraPreset)?.label || '🏙 Isometric'} ▼
          </button>
          {presetOpen && (
            <div style={{ position: 'absolute', top: '100%', left: 0, marginTop: '4px', background: 'var(--bg-panel)', border: '1px solid var(--border-subtle)', borderRadius: '8px', padding: '4px', width: '200px', boxShadow: 'var(--shadow-md)', zIndex: 30 }}>
              {PRESETS.map(p => (
        ...
        <button onClick={onResetCamera} style={{ backgroundColor: 'var(--bg-elevated)', color: 'var(--text-primary)', border: '1px solid var(--border-default)' }}>
          ⟳ Reset Camera
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          />
          <span style={{ fontSize: '12px', color: 'var(--text-muted)', fontFamily: 'monospace', width: '30px' }}>{explodeValue}</span>
        </div>
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
        ...
        <button style={{ backgroundColor: 'var(--bg-elevated)', color: 'var(--text-primary)', border: '1px solid var(--border-default)', padding: '8px', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Fullscreen">
          ⛶
        </button>
      </div>
    </div>
